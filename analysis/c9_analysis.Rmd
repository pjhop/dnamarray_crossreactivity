...
library(bacon) # BioConductor (BiocManager::install("bacon"))
library(FDb.InfiniumMethylation.hg19) # BioConductor (BiocManager::install("FDb.InfiniumMethylation.hg19"))
...
# Save plots
save_plots <- FALSE # if TRUE, plot objects won't be removed, and plots can be saved at the end of this markdown (this does require quite some RAM, since some of the plot objects are quite large)
...
Downstream analyses corresponding to the manuscript: "Crossreactive probes on Illumina DNA methylation arrays: a large study on ALS shows that a cautionary approach is warranted in interpreting epigenome-wide association studies". 
...
# Annotation
anno <- readr::read_tsv("../data/extdata/anno_450k.txt.gz")
...
rm(matches_methylated_nomismatch_final_max5)
...
rm(matches_unmethylated_nomismatch_final_max5)
...
rm(matches_YpG_nomismatch_final_max5)
...
## Other EWAS methods
ewas_meffil <- readRDS("../data/output/ewas/ewas_meffil_c9_missing0.05.rds")
meffil_isva <- tibble(Probe = rownames(ewas_meffil$p.value), p = ewas_meffil$p.value[,"isva"])
meffil_sva <- tibble(Probe = rownames(ewas_meffil$p.value), p = ewas_meffil$p.value[,"sva"])

mlma_moment <- read_tsv("../data/output/ewas/mlma_moment_c9_covarsexbatch_qcovaragesmoking_missing0.05.mlma")
mlma_moment <- mlma_moment %>% dplyr::mutate(t = b / se)

mlma_moa <- read_tsv("../data/output/ewas/mlma_moa_c9_covarsexbatch_qcovaragesmoking_missing0.05.moa")
mlma_moa <- mlma_moa %>% dplyr::mutate(t = b / se)

baconize <- function(stats, df) {
...
mlma_linear_bacon <- baconize(mlma_linear, df = max(mlma_linear$NMISS) - 20)
...
rm(mlma_meta, mlma_mvalues, mlma_rmPCAoutliers, fdb, beta_)
# Clean up 
if(!save_plots) {rm(qqplot, manhattan, compare_rmPCAoutliers, compare_linear, compare_meta, compare_mvalues, plots_c9, plots_trans_chr9, plots_trans_nochr9, heatmap)}
```
...
```{r, include = FALSE, cache = FALSE}
rm(diff_methylated_nomismatch_mismatch_indel)
# Clean up 
if(!save_plots) {rm(plot_typeI_nomismatch_noindel_correlated_methylated, plot_typeI_nomismatch_noindel_correlated_methylated_coloredswitch,
manhattan_oob, qqplot_oob, plot_typeI_nomismatch_noindel_correlated_methylated_oobpvalues, diff_plot_nomismatch_mismatch_indel,
plot_typeI_mismatch_indel_correlated_methylated_oobpvalues, plot_typeI_mismatch_indel_methylated_tstat, plot_typeII_nomismatch_noindel_correlated_methylated, plot_typeII_mismatch_indel_correlated_methylated, manhattan_hlightcrossreactive, manhattan_oob_hlightcrossreactive, ORs_plot, plot_typeI_mismatch_indel_correlated_methylated_reference, diff_plot_nomismatch_mismatch_indel_legend)}
```

## Number of spurious probes across varying EWAS methods

```{r}
mlma_loco_sig <- mlma %>%
  filter(p < 0.05/nrow(.)) %>%
  mutate(cr_c9 = Probe %in% crossreactive_probes_combined)
mlma_moa_sig <- mlma_moa %>%
  filter(p < 0.05/nrow(.)) %>%
  mutate(cr_c9 = Probe %in% crossreactive_probes_combined)
mlma_moment_sig <- mlma_moment %>%
  filter(p < 0.05/nrow(.)) %>%
  mutate(cr_c9 = Probe %in% crossreactive_probes_combined)
mlma_linear_sig <- mlma_linear %>%
  filter(p < 0.05/nrow(.)) %>%
  mutate(cr_c9 = Probe %in% crossreactive_probes_combined)
mlma_linear_bacon_sig <- mlma_linear_bacon %>%
  filter(p < 0.05/nrow(.)) %>%
  mutate(cr_c9 = Probe %in% crossreactive_probes_combined)
meffil_isva_sig <- meffil_isva %>%
  filter(p < 0.05/nrow(.)) %>%
  mutate(cr_c9 = Probe %in% crossreactive_probes_combined)
meffil_sva_sig <- meffil_sva %>%
  filter(p < 0.05/nrow(.)) %>%
  mutate(cr_c9 = Probe %in% crossreactive_probes_combined)

ewas_methods <- tibble(
  ewas_method = c("OSCA_LOCO", "OSCA_MOA", "OSCA_MOMENT", "Linear", "Linear_bacon", "SVA", "ISVA"),
  `# Significant` = c(nrow(mlma_loco_sig), nrow(mlma_moa_sig), nrow(mlma_moment_sig), nrow(mlma_linear_sig), nrow(mlma_linear_bacon_sig),
            nrow(meffil_sva_sig), nrow(meffil_isva_sig)
            ),
  `N Crossreactive` = c(sum(mlma_loco_sig$cr_c9), sum(mlma_moa_sig$cr_c9), sum(mlma_moment_sig$cr_c9), sum(mlma_linear_sig$cr_c9), sum(mlma_linear_bacon_sig$cr_c9),
           sum(meffil_sva_sig$cr_c9), sum(meffil_isva_sig$cr_c9)
           ),
  `% Crossreactive` = paste0(signif((`N Crossreactive`/`# Significant`)*100, 3), "%")
)
ewas_methods
```
...
```{r, include = FALSE, cache = FALSE}
rm(test_methylated_unmethylated_mismatch_indel_typeII, test_methylated_unmethylated_mismatch_indel2,
   mlma_moa, meffil_sva, meffil_isva, mlma_linear, mlma_linear_bacon, mlma_moment,
   mlma_loco_sig, mlma_moa_sig, mlma_moment_sig, mlma_linear_sig, mlma_linear_bacon_sig)

# Clean up 
if(!save_plots) {rm(diff_plot_unmethylated_methylated_typeII, diff_plot_unmethylated_methylated_typeI)}
```
...
```{r, include = FALSE, cache = FALSE}
rm(test_methylated_YpG_mismatch_indel2, test_unmethylated_YpG_mismatch_indel2)
# Clean up 
if(!save_plots) {rm(diff_plot_methylated_YpG_mismatch_indel2_2, diff_plot_unmethylated_YpG_mismatch_indel2_2)}
```
...
```{r, include = FALSE, cache = FALSE}
rm(test_typeII_YpG_mismatch_indel2_unmethylated, test_typeII_YpG_mismatch_indel2_methylated, matches_YpG_mismatch_indel_final, matches_YpG_nomismatch_final)
# Clean up 
if(!save_plots) {rm(diff_plot_typeII_methylated_YpG_mismatch_indel2_2, diff_plot_typeII_unmethylated_YpG_mismatch_indel2_2)}
```
...
rm(matches_methylated_nomismatch_includeflanking_final_max5)
...
rm(matches_unmethylated_nomismatch_includeflanking_final_max5)
...
```{r, include = FALSE, cache = FALSE}
rm(diff_flanking_methylated, matches_methylated_nomismatch_includeflanking_final)
# Clean up 
if(!save_plots) {rm(diff_plot_flanking_methylated)}
```
...
```
```{r, include = FALSE, cache = FALSE}
rm(diff_flanking_both, matches_methylated_unmethylated_mismatch_indel_includeflanking_final, matches_methylated_mismatch_indel_includeflanking_final, matches_unmethylated_nomismatch_includeflanking_final)
# Clean up 
if(!save_plots) {rm(diff_plot_flanking_both)}
```
...
diff_0bp_both <- matches_methylated_unmethylated_mismatch_indel_final  %>%
  dplyr::left_join(matches_methylated_unmethylated_mismatch_indel_0bp_final %>% dplyr::select(Probe, Type2, width), 
                   by = "Probe") %>%
  dplyr::mutate(diff = width.y-width.x,
                id = paste(Probe, strand, width.x, width.y, start, end,sequence_bs, sbe_site,Type2.x,Type2.y, sep ="_")
                ) %>%
...
```{r, include = FALSE, cache=FALSE}
rm(diff_0bp_both, matches_methylated_unmethylated_mismatch_indel_0bp_final, matches_unmethylated_mismatch_indel_0bp_final, matches_methylated_mismatch_indel_0bp_final, matches_methylated_mismatch_indel_0bp_final_14bp, matches_unmethylated_mismatch_indel_0bp_final_14bp)
if(!save_plots) {rm(diff_plot_0bp_both)}
```
...
# annotation
annoEPIC <- readr::read_tsv("../data/extdata/anno_EPIC.txt.gz")
