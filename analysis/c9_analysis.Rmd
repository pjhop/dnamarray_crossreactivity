...
# Zhou 
zhou <- read_tsv("../data/extdata/hm450.hg38.manifest.tsv.gz")
...
plot_typeI_mismatch_indel_correlated_methylated_reference <- matches_methylated_mismatch_indel_final %>% 
  dplyr::mutate(previously_flagged = ifelse(Probe %in% c(chen$TargetID, zhou$probeID, naeem$probe), TRUE, FALSE)) %>%
  dplyr::filter(!is.na(cistrans)) %>%
  dplyr::filter(Type == "I") %>%
...
crossreactive_reference <- unique(c(chen$TargetID, zhou$probeID, naeem$probe))
...
tab_reference <- tibble(reference = c("Chen et al.", "Naeem et al.", "Zhou et al.", "Total"), 
              nr_crossreactive = c(sum(crossreactive_probes_methylated %in% chen$TargetID), 
                                   ...
                                   sum(crossreactive_probes_methylated %in% zhou$probeID),
              ...
              nr_crossreactive_sig = c(sum(mlma_sig_crossreactive$Probe %in% chen$TargetID),
                                       ...
                                       sum(mlma_sig_crossreactive$Probe %in% zhou$probeID),
...
beta_oob_sig_total_sig <- beta_oob[rownames(beta_oob) %in% total_oob_sig$Probe & rownames(beta_oob) %in% crossreactive_probes_combined,samplesheet$Sample_Name]
samplesheet_ib <- cbind(samplesheet, t(beta_sig_total_sig))
samplesheet_oob <- cbind(samplesheet, t(beta_oob_sig_total_sig))
...
beta_plot_ib <- samplesheet_ib %>%
  ...
  tidyr::gather(key = "Probe", value = "beta", dplyr::starts_with("cg")) %>%
...
# Samplesheet
samplesheet_EPIC <- readRDS("../data/raw/samplesheet_UK_2017.06_383samples_Ireland_2017.03_298samples_cleaned.rds")
...
We performed a replication analysis in an EPIC dataset consisting of `r sum(samplesheet_EPIC$C9ORF72.Status == "expanded")` expanded cases and `r sum(samplesheet_EPIC$C9ORF72.Status == "normal")` non-expanded cases. 
